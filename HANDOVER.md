# RSG Copywriting Companion — Claude Code Handover

## What this is

A working prototype of a conversational copywriting tool for the Rivers Casino & Resorts (RSG) creative team. Built for an internal demo with Doner Advertising's creative team to illustrate the principle: a context-aware writing companion that knows what each RSG property sounds like and helps writers produce on-brand copy faster.

The prototype covers two RSG properties: **Pittsburgh** and **Des Plaines**. These two properties were chosen because they represent materially different creative operating modes — Pittsburgh's voice lives in platform identities and visual systems with relatively functional headline copy, while Des Plaines' voice lives in the headlines themselves with strong pun-on-mechanic craft. Showing the tool can handle both at once proves the methodology generalizes.

## Scope: what to build

A single-page web application with:
1. A chat interface where the user can paste or type input and have a conversation
2. Real Claude API integration on the backend
3. A property selector (Pittsburgh / Des Plaines) at the top of the chat
4. A long-form system prompt that encodes both properties' voice, recurring phrases, platform identities, local hooks, and format rules
5. No persistence — each session is fresh, no memory across sessions, no login

The user opens it, picks a property, drops in a brief or types a description of the work they need help with, and has a conversation that produces headline directions, body copy, format-specific variants, etc.

## Scope: what NOT to build

Be explicit about these boundaries so the prototype stays focused:

- **No memory across sessions.** Stateless. Closing and reopening the browser starts fresh.
- **No Brand Gravity encoding.** This is a separate downstream workstream not yet ready. The system prompt is the encoded layer for now.
- **No retrieval-augmented generation (RAG) over the corpus.** The encoded layer is in the system prompt as text. We do not need a vector database, embedding system, or document retrieval layer.
- **No image generation.** Output is text only.
- **No production deployment infrastructure.** Local development is fine. If hosting is needed for the demo, a simple Vercel deploy is fine; if the team wants to run it locally, that's also fine.
- **No user accounts, auth, or saved conversations.**
- **No translation, multilingual support, or generation of non-English copy.** If a writer asks about Lunar New Year copy for a property that produces multilingual versions, the tool flags that translation is needed but does not generate translated copy.
- **No form-and-output UI patterns.** This is a chat interface. No required dropdowns, no structured input fields beyond the property selector. The user types or pastes whatever they have.
- **No pun-or-direct toggle, no register selector, no work-type dropdown.** The voice register and wordplay (or absence) emerge naturally from what the system prompt knows about the property and the work. The user does not pick.

## Tech stack

Recommended stack, but use your judgment if there's a better option:

- **Frontend:** Next.js (App Router) with TypeScript and Tailwind. Single-page app with chat interface.
- **Backend:** Next.js API route that calls the Anthropic SDK
- **Anthropic SDK:** `@anthropic-ai/sdk` latest version
- **Model:** `claude-sonnet-4-5` (the latest Sonnet at time of build — verify the exact model string when you build)
- **API key:** `ANTHROPIC_API_KEY` environment variable. Document in README that the user needs to set this.
- **No database, no auth, no state management library.** React useState is sufficient.

If you decide a different stack makes more sense (e.g., a single-file React app with Vite, or a simple Python Flask app), that's fine. The constraint is that it has to run locally with minimal setup and call the Claude API.

## Functional requirements

### Property selector
A simple toggle or dropdown at the top of the page: "Pittsburgh" or "Des Plaines". Selecting a property loads that property's voice into the system prompt for that conversation. Switching properties mid-conversation should warn the user that the conversation will reset (or just reset it — pick whichever feels less janky).

### Chat interface
Standard chat UI patterns. Message history visible. User input at the bottom. Send on Enter, Shift+Enter for newline. Loading indicator while waiting for response.

The user's first message can be anything: a pasted Workfront ticket, a typed description, a one-line prompt, a list of deliverables. The system prompt instructs Claude to parse what's there, ask for missing context if something material is unclear, and propose drafts.

Subsequent messages are conversational. The user might ask for revisions ("more like the second one but punchier"), more options ("give me three more"), format variants ("now do the body copy at 25 words for the postcard back"), or to change direction ("what if we leaned into the March Madness angle?").

### System prompt
The system prompt is the encoded voice layer. It contains:
1. A general framing of what the tool is and how it should operate (assist, not generate; the user owns the judgment)
2. Pittsburgh-specific encoding: voice register, recurring phrases, platform identities, local hooks, format rules
3. Des Plaines-specific encoding: same structure
4. Operational rules: how to handle missing context, how to propose drafts, when to ask clarifying questions, how to format output

The system prompt is in PART 2 of this handover document. It needs to be loaded based on which property the user has selected. You can either (a) load the full prompt with both properties and tell Claude which property is active in the user message, or (b) construct the system prompt dynamically per conversation. Either works — the second is cleaner.

### Output format
The tool should produce conversational responses in plain text and markdown. Headline directions can be presented as a short list. Body copy can be presented as a paragraph. Format-specific variants should be labeled. The tool should NOT produce JSON, structured data, or tables unless the user asks for them.

### Error handling
If the API call fails, show a friendly error message and let the user retry. If the user's input is empty or trivially short, the tool should ask for more context rather than fail. If the user pastes something that isn't related to RSG copy work, the tool should politely redirect.
...
2. **`SYSTEM_PROMPT.md`** — the encoded voice layer (PART 2 of this document, copy verbatim into a separate file for the build)
3. **`transcript_pittsburgh.md`** — full Pittsburgh transcription (reference material; not needed at runtime, but useful for the build team to understand the source material)
4. **`transcript_des_plaines.md`** — full Des Plaines transcription (same)
5. **`transcript_philadelphia.md`**, **`transcript_schenectady.md`**, **`transcript_portsmouth.md`** — the other three property transcripts (not used in this prototype but useful context if anyone needs to understand the broader pattern)
6. **`README.md`** — basic setup instructions (install deps, set env var, run dev server)

## Build sequence suggestion

1. Set up the Next.js project with Tailwind
2. Build the chat UI with hardcoded fake responses to verify the interaction patterns work
3. Wire up the Anthropic SDK and replace fake responses with real API calls
4. Load the system prompt from SYSTEM_PROMPT.md based on selected property
5. Test with three or four real briefs (sample briefs in PART 3 below)
6. Iterate on the system prompt if outputs don't feel right — the prompt is the product

## Acceptance criteria

The prototype is "done" when:

- A user can pick Pittsburgh or Des Plaines, type or paste a brief, and have a conversation that produces headline directions
- The drafts feel like they belong at the selected property (Pittsburgh drafts feel like Pittsburgh, DP drafts feel like DP)
- The user can ask for revisions, more options, format variants, and the tool stays in the conversation
- The tool asks for clarifying context when something material is missing
- A team member can use it during the retro tomorrow with their own brief and get a useful result

This is a prototype, not production software. It can be rough around the edges. The point is to illustrate the principle for the team.
---

# PART 2 — System Prompt (Encoded Voice Layer)

The text below is the system prompt for the Claude API call. Load it as the system parameter on every request. The active property should be substituted in or referenced in the user message ("I'm working on a Pittsburgh piece" / "I'm working on a Des Plaines piece") so Claude knows which property's voice to operate from.

The system prompt is long because the encoding needs to be specific. The team will be able to tell the difference between a tool that knows RSG and a tool making generic casino copy.
...
# PART 3 — Sample Briefs for Testing

These are real-feeling briefs the build team can use to test the prototype during development and that you can use to demo to the creative team. Each one matches a typical Workfront ticket shape — sparse, partial, sometimes ambiguous. The tool should be able to handle all of them.

## Test brief 1: Pittsburgh, well-formed

```
Property: Pittsburgh
Job: March 26 Mystery Gift Card Giveaway
Deliverables:
- Postcard 9x6 (front + back)
- Kiosk tile 320x217
- Digital slide 1280x720
Mechanic: Recurring monthly Mystery Gift Card Giveaway. One ??? card revealed at the kiosk on Sundays in March. Various retailer gift cards as prizes.
```

Expected behavior: The tool should recognize this as a Pittsburgh Mystery Gift Card piece, default to the Memphis Q1/Q2 quarterly theme, propose 3-5 headline directions that fit Pittsburgh's platform-and-system register, and offer to do format-specific variants when asked.

## Test brief 2: Des Plaines, sparse one-liner

```
DP, May Cinco de Mayo Slot Play, postcard front + back
```

Expected behavior: The tool should recognize this as Des Plaines Cinco de Mayo work, propose headline directions that lean into DP's pun-on-mechanic register and the existing Cinco de Mayo platform (papel picado motif, cultural craft), and produce body copy when asked.

## Test brief 3: Pittsburgh, ambiguous platform

```
PGH — Forever Fun Hot Seat Drawings for April. Need digital slide and postcard.
```

Expected behavior: The tool should recognize Forever Fun Club at Pittsburgh, know it's a 55+ club with weekly hot seat drawings, propose headlines in the property's register (functional but with the seasonal April moment if that's a hook), and be aware that this platform also runs at PHL and POR (cross-property platform).

## Test brief 4: Des Plaines, restaurant-specific

```
Des Plaines, Hugo's Frog Bar dining offer DM for Mother's Day. Self-mailer.
```

Expected behavior: The tool should recognize Hugo's as a DP restaurant with its own brand identity (golden serif type, frog mascot, steakhouse register), and Mother's Day as a seasonal moment. Proposed headlines should fit the steakhouse register with a Mother's Day hook. Should NOT use the Mian "Delicious flavors await" or RC Supply "Rewards you can taste" phrases (those belong to those restaurants, not Hugo's).

## Test brief 5: Pittsburgh, the NFL Draft moment

```
Pittsburgh — looking for a take on the NFL Draft. April 22-25, Pittsburgh hosts. We want to do something for it.
```

Expected behavior: The tool should treat this as front-of-funnel concepting (closer to Tool 2 territory but should still be useful). Propose 3-5 directions that use the local-moment authentically — "You're on the clock" framing, draft-card iconography possibilities, Pittsburgh-hosting framing — without overcommitting to a specific platform identity. Should ask whether this is for an existing platform (Take It or Risk It Fridays?) or a new one.

## Test brief 6: Des Plaines, paste from Workfront

```
Project Name: April 26 Slot Core DM
Property: Des Plaines
Deliverables: Self-mailer 8.5x11
Request Background: [empty]
Notes: Standard April recurring slot play DM. Personalization fields: <FirstName>, <$X>, <Patron ID>.
```

Expected behavior: The tool should recognize this as workhorse DM territory, default to one of DP's recurring phrase library options ("Thrills await" / "Don't keep the thrills waiting" / "Pick your day to play"), and produce front + interior + body copy in the standard DP self-mailer structure. Should adapt copy length to 8.5x11 self-mailer format.

## Test brief 7: Wrong property, wrong platform

```
Pittsburgh, Mystery Fridays for March
```

Expected behavior: The tool should recognize that "Mystery Fridays" is the Philadelphia platform and that Pittsburgh runs the riff "Take It or Risk It Fridays" instead. It should ask the writer if they meant Take It or Risk It (and proceed if yes), or if they actually want Pittsburgh to use Philly's Mystery Fridays mark (and flag this as outside the encoded patterns).

---

# How to use this handover

1. Read PART 1 to understand the build spec
2. Copy PART 2 (the system prompt) verbatim into a separate file in the project (`SYSTEM_PROMPT.md`)
3. Drop the five property transcripts into the project for reference (especially `transcript_pittsburgh.md` and `transcript_des_plaines.md`)
4. Build the prototype following the build sequence in PART 1
5. Test against the sample briefs in PART 3
6. Iterate on the system prompt if the outputs don't feel right — the prompt is the product

The whole thing should be buildable in a few hours of focused work, especially if you start from a Next.js + Tailwind starter.

If something in the system prompt is unclear or needs more detail, the property transcripts at `transcript_pittsburgh.md` and `transcript_des_plaines.md` have the source material the system prompt was distilled from. Those files contain item-by-item visible copy from the actual creative work.
